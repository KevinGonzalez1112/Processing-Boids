...
class Boid 
{
  ...
  PVector align(ArrayList<Boid> boids)
  {
    ...
    {
      float d = PVector.dist(position, other.position);
      if ((d > 0) && (d < neighborDist) && (other.col == col))
      {
  ...
  void run(ArrayList<Boid> boids)
  {
    if (unpause)
    {
      ...
      selectedPixel = backingImage.get((int)position.x, (int)position.y);

      // Create Vectors for the Boids colour and the selected pixel colour
      PVector boidC = new PVector(red(col), green(col), blue(col));
      PVector pixelC = new PVector(red(selectedPixel), green(selectedPixel), blue(selectedPixel));
      
      // Normalize both vectors to a length of 1
      boidC.normalize();
      pixelC.normalize();
      
      // Gives a number between 0 and 180
      float theta = 180.0 * acos(boidC.dot(pixelC))/PI;
      
      // If angle difference between the Vectors is less than 10
      // Then boid will draw that pixel on the canvas
      if (theta < 10)
      {
        // Translate the pixel over to Boids drawing
        boidsDrawing.set((int)position.x, (int)position.y, selectedPixel);
      }
    }
    render();
  }
